...
def get_std1(X_test, Y_test, my_model,acc):
    #存储所有标准差
    dss = []
    ...
    for j in range(30,181,5):
        ...
        for index in range(50):
            ...
            first = np.random.randint(0, len(dense_output)-1)
            print("first", first)
            #adaptive random
            X_test2.append(X_test[first])
            if (first==0 and dis[first][1]==0) or (first!=0 and dis[first][0]==0):
                for first_i in range(len(dense_output)):
                    dis[first][first_i] = math.sqrt(np.power(dense_output[first] - dense_output[first_i], 2).sum())


            X_test2.append(X_test[np.argmax(dis[first])])
            second = np.argmax(dis[first])
            if (second == 0 and dis[second][1] == 0) or (second != 0 and dis[second][0] == 0):
                for second_i in range(len(dense_output)):
                    dis[second][second_i] = math.sqrt(np.power(dense_output[second] - dense_output[second_i], 2).sum())

            Y_test2.append(Y_test[first])
            Y_test2.append(Y_test[np.argmax(dis[first])])

            pre_num = []
            pre_num.append(first)
            pre_num.append(np.argmax(dis[first]))
            while len(X_test2) < j:
                mins = []
                ...
                maxnum = 0
                X_test2.append(X_test[mins[0][1]])
                Y_test2.append(Y_test[mins[0][1]])
                pre_num.append(mins[0][1])
                for i in mins:
                    if i[0] > maxnum:
                        X_test2[-1] = X_test[i[1]]
                        Y_test2[-1] = Y_test[i[1]]
                        pre_num[-1] = i[1]
                        # pre_num.append(i[1])
                temp = pre_num[-1]
                if (temp == 0 and dis[temp][1] == 0) or (temp != 0 and dis[temp][0] == 0):
                    for temp_i in range(len(dense_output)):
                        dis[temp][temp_i] = math.sqrt(np.power(dense_output[temp] - dense_output[temp_i], 2).sum())
...
if __name__=="__main__":
    ...
    #for sample_num in range(console_flags.min_samples[0],console_flags.min_samples[1]+1,15):
    ...
    #
    # #计算异常点每两点之间的距离
    dis = np.zeros((len(dense_output), len(dense_output)))
    ...
    for a_unoise in arange(0.8,0.9,0.1):
        print(a_unoise)
        dss = get_std1(X_test=X_test, Y_test=Y_test, my_model=my_model, acc=acc)
    ...
    if console_flags.dec_dim is None:
        workbook.save(os.path.join(basedir, "resultmutant2","{}-sn{}-cs{}.xlsx".format(exp_id, min_samples, min_cluster_size)))
    else:
        workbook.save(os.path.join(basedir, "resultmutant2","{}-sn{}-cs{}-dim{}.xlsx".format(exp_id, min_samples,
                                                                                          min_cluster_size,
