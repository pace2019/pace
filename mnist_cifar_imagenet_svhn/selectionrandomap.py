...
def get_std1(X_test, Y_test, a_unoise, countlist,res,label_noise, first_noise,res_get_s,my_model,acc):
    ...
    for j in range(30,181,5):
        ...
        for c in range(50):
            X_test2 = X_test2[:int(len_noise)+1]
            Y_test2 = Y_test2[:int(len_noise)+1]
            print(len(X_test2))
            get_ds_random(countlist, res_get_s, j*a_unoise, X_test, Y_test, X_test2, Y_test2, res, ds, my_model,acc)
...
if __name__=="__main__":
    ...
    if console_flags.dec_dim is None:
        workbook.save(os.path.join(basedir, "resultrandomap","{}-sn{}-cs{}.xlsx".format(exp_id, min_samples, min_cluster_size)))
    else:
        workbook.save(os.path.join(basedir, "resultrandomap","{}-sn{}-cs{}-dim{}.xlsx".format(exp_id, min_samples,
                                                                                          min_cluster_size,
