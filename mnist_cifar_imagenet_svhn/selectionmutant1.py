...
def get_ds(mmd_res, sample_size,X_test,Y_test, X_test2, Y_test2, ds, my_model,acc):
    ...
    for num in range(sample_size):
        X_test2.append(X_test[mmd_res[num]])
        Y_test2.append(Y_test[mmd_res[num]])
    #新构造的采样测试集
...
def get_std1(X_test, Y_test, my_model,acc):
    ...
    for j in range(30,181,5):
        ...
        get_ds(mmd_res, j, X_test, Y_test, X_test2, Y_test2,  ds, my_model,acc)
...
if __name__=="__main__":
    ...
    mmd_res, _ = run(dense_output, Y_test, gamma=0.026, m=min(len(dense_output), 180), k=0, ktype=0, outfig=None,
    ...
    if console_flags.dec_dim is None:
        workbook.save(os.path.join(basedir, "resultmutant1","{}-sn{}-cs{}.xlsx".format(exp_id, min_samples, min_cluster_size)))
    else:
        workbook.save(os.path.join(basedir, "resultmutant1","{}-sn{}-cs{}-dim{}.xlsx".format(exp_id, min_samples,
                                                                                          min_cluster_size,
